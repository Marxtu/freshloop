...
- **Start a run anywhere** — a home **address search with live autocomplete** (OSM Nominatim, **nearby-first** — type "Carrefour" and the dropdown lists the ones *near you*, each with its **straight-line distance from you**, with a global fallback) plus a **GPS "locate me"** button set the start point from your real location, not a fixed city centre.
