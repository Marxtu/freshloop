...
4. **"Multi-device" support** — multi-device adaptation (quotes = "decent adaptive layout suffices")
...
- Two app ideas: an IDTM tent-installation field guide (with **visual recognition** of items and installation mistakes); a health-facility assessment tool (select disease → structured survey → **3-tier scoring (Meets +++ / Partial ++ / Does not meet +)** → **downloadable report** + suggested improvements).
- Expectation signal: serious offline-capable field tooling, computer vision, structured domain data, scoring engine, report generation → the most technically demanding of the four cases.
- **Reusable**: WHO's "3-tier scoring + downloadable report" pattern → directly applicable to FreshLoop's route-quality scoring (turn the score from a black box into structured, explainable badges + a report export).
...
- **Only GPS/maps adjacency to our project**: MoveAble proves that "live GPS + map UI + periodic position publishing" is an accepted, expected pattern (but a completely different domain — no collision).
...
- Explicitly allows: "choose our idea, a variation, **or a completely different idea**; language (Swift/Kotlin) cannot change once chosen" — **this confirms a self-chosen topic like FreshLoop is allowed**.
...
- **This is the grading template**: a focused core flow + a real external API + platform-extension polish (widget, wearable, haptics, animations, notifications, personalization, statistics). FreshLoop's complexity should match or slightly exceed these cards.

### 2.5 Collision & originality conclusion

- **No sponsor case collides with running/route/fitness** → safe against red line 2 (already-existing app) relative to these cases.
...
- The RN deck **teaches no maps/geolocation at all** (no react-native-maps, no expo-location) → no head start for a map+GPS-heavy app.
- The native decks **also teach no GPS/maps APIs** (CoreLocation/MapKit appear only as boxes in architecture diagrams; Android only mentions `LocationManager` once, for lifecycle) → maps/location are "self-taught" territory for any stack.
...
| Cross-screen shared state (routes↔map↔tracking) | **`Cubit`/`flutter_bloc`** (his sanctioned app-state solution; **not Riverpod/Provider — not taught**) |
...
| Test campaign | `test` (scoring algo is pure Dart — ideal unit target) + `flutter_test`/`WidgetTester` (widget tests); `integration_test` as a value-add |

### 3.4 Not covered by the decks — pull as "external package/service" (which is itself a grading plus)

- **Map rendering**: not taught → pull from pub.dev (the professor's framing is "use only official/pub.dev packages, add via `flutter pub add`", which legitimizes it). Choose **`flutter_map` + OSM tiles** (free, no key; avoids google_maps_flutter's billing-enabled Google key).
...
- **Multi-device** = width/size-class-driven adaptive layouts + phone↔tablet (Android `BoxWithConstraints`, iOS size classes) → Flutter equivalent via `LayoutBuilder`/`MediaQuery`; **must build genuine width-adaptive layouts, not just scale up the phone layout**.
- **Look & feel** = Material 3 Color/Typography/Shape centralized theme → Flutter `ThemeData` maps directly.
- **Test campaign** = the Android testing pyramid (unit + instrumented/Compose UI tests, Finders/Assertions/Actions) → Flutter `flutter_test` (unit/widget) + `integration_test` map almost 1:1; plan all three layers.
...
- `.cc-connect/attachments/extracted/{who,treNord,tim,bendingSpoons}.pdf` — 4 sponsor cases
- `.cc-connect/attachments/extracted/{3-flutter,4-reactNative,5-android,6-ios}.pdf` — 4 framework decks
