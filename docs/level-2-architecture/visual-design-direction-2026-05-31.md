...
**"Fresh-air cartographic":** the map is the hero, crisp surfaces float over it, and one vivid trail-green leads with a warm energetic accent reserved for the primary action and the top score. Sporty and outdoors, never corporate-generic.

- **Tone:** energetic, outdoorsy, confident, clean. Photography-forward (scenery photos are visual anchors, which directly serves the professor's "pictures are faster than words").
- **The one memorable thing:** opening a route *draws the loop onto the map* and the score badges settle in, so the route feels designed *for you*, reinforcing "decide for me, but let me have the final say."
...
- **Body / labels:** **DM Sans**, clean and legible, slightly characterful; comfortable at small sizes and large dynamic-type sizes.
...
Built on Material 3 `ColorScheme.fromSeed` for tonal coherence + guaranteed contrast, but with **chosen** colors rather than the default.

- **Primary (seed): vivid trail green** `#0E9F6E` (fresh and energetic, brighter than the muted forest green of the M1 placeholder; update the seed in `theme.dart`).
...
- No Roboto/Inter/Arial/system-default fonts, no purple-on-white gradients, no flat gray cookie-cutter cards, no generic dashboard look.
- No oceans of buttons, no idiot boxes, no chart junk, no metaphor mismatch (the professor's 4 anti-patterns; see the UX checklist §3).
...
- **`frontend-design`** (Anthropic) is the source method here: commit to a bold, intentional aesthetic; distinctive (non-default) typography; dominant color + sharp accent; purposeful motion; avoid generic AI aesthetics. Applied.
- **`theme-factory`** (Anthropic, 10 pro themes): its nature presets ("Forest Canopy / earth tones", "Botanical Garden / fresh organic") validate the green-led palette as an established professional choice, and its discipline (one cohesive palette + font pairing, applied consistently, contrast-checked) matches §2–§3. Also adopted its **"confirm the theme with the user before applying"** step.
- **`brand-guidelines`** (Anthropic, brand-specific) isn't directly applicable (it's Anthropic's own identity), but it confirms the right structure: defined colors + heading/body font pairing + a reserved accent + contrast-aware text. Mirrored.
...
> Note: `frontend-design` targets web (HTML/CSS); only its *design judgment* is applied here, since the implementation is Flutter/Material 3. This direction is a starting commitment, refined as real screens are screenshot-reviewed.
