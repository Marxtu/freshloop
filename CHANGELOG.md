...
- **Start a run anywhere** — home **address search with live autocomplete** (OSM Nominatim, **nearby-first** with a global fallback, debounced; each suggestion shows its **straight-line distance from you**) + a **GPS "locate me"** button set the start from the runner's real position instead of a fixed city centre.
